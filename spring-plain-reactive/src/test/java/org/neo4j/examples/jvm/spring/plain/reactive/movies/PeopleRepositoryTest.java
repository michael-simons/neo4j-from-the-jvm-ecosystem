 ...
 */
@DataNeo4jTest(includeFilters = @ComponentScan.Filter(type = FilterType.ANNOTATION, classes = Repository.class))
class PeopleRepositoryTest {
	...
	void getDetailsByNameShouldWork(@Autowired PeopleRepository peopleRepository) {
		...
		peopleRepository.getDetailsByName("Keanu Reeves")
			.as(StepVerifier::create)
			.consumeNextWith(personDetails -> {
		...
		peopleRepository.getDetailsByName("foobar")
			.as(StepVerifier::create)
			.verifyComplete();
	}
